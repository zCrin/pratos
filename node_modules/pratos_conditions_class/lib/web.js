		...
		User.verify_connection(req.user_id,globalVariable , function(user_res){
			if(user_res == true){
				fs.readFile(pathTo +'/recipes.json','utf8', function(err, result){
					...
					if(req.body.recipeId && req.body.recipeId!="0"){
						data.recipes[req.body.recipeId] = req.body.name;
							fs.writeFile(pathTo+'/recipes/'+ req.body.recipeId +".js", req.body.code,function(){
								console.log(("\nSystem : updating recipe " +req.body.name + " : .js file written").yellow)
							});
							fs.writeFile(pathTo +'/recipes/'+ req.body.recipeId +".xml", req.body.workspace,function(){
								console.log(("\nSystem : updating recipe " +req.body.name + " : .xml file written").yellow)
							});
							fs.writeFile(pathTo +'/recipes.json',JSON.stringify(data),function(){
								console.log(("\nSystem : saving recipe " +req.body.name + " change").yellow)
							});
							res.end('1');
					}
					else{
						...
						var filename = uuidV1();
						data.recipes[filename] = req.body.name;
						fs.writeFile(pathTo +'/recipes.json',JSON.stringify(data),function(){
								console.log(("\nSystem : registering recipe " +req.body.name).yellow)
							});
						fs.writeFile(pathTo +'/recipes/'+ filename +".xml", req.body.workspace,function(){
								console.log(("\nSystem : updating recipe " +req.body.name + " : .xml file written").yellow)
							});
						fs.writeFile(pathTo +'/recipes/'+ filename +".js", req.body.code,function(){
								console.log(("\nSystem : updating recipe " +req.body.name + " : .js file written").yellow)
							});
						
						cmd.get("sudo chmod -R 777 "+pathTo +'/recipes/',function(){
							console.log(("\nSystem : setting recipe file CHMOD to 77").yellow)
						});
						res.end('1');
					}
				});
...
fs.readFile(pathTo +'/recipes/'+ req.body.recipeId +".xml",'utf8', function(err, result){
						res.end(result);
			...
			if(user_res == true ){
				if(req.body.recipeId){
					...
					fs.readFile(pathTo+'/recipes.json','utf8', function(err, result){
						...
						var data = JSON.parse(result);
if(data.recipes[req.body.recipeId]){
	var name = data.recipes[req.body.recipeId];
					var filename = req.body.recipeId;
delete data.recipes[req.body.recipeId];
