...
watchFile.close();
			...
			watchFile=	fs.watch(path+'/log/homebridge.log', 
					function(data) {
						if(data =='change'){
...
cmd.get("tail -n 1 /var/www/pratos/log/homebridge.log",
								function (res) {
									if(res){
										...
										if(resArray[2] && !/Initializing|Getting|ECONNREFUSED|Requesting|HTTP bad response|HTTP get power function failed|HTTP set power function failed/i.test(resArray[2]) && resArray[2].trim() != ""){
											console.log(res);
...
delete_rooms:function(user_id, globalVariable,callback){
	...
	list_accessories: function(globalVariable, callback,specificOne,specificAid){
...
if(specificOne && specificAid==aid ||! specificOne){
...
answer += `, "room" : "` + updated_accessories[aid].room + '"';
						...
						var responseD = JSON.parse(answer.slice(0, -1) +  ']}');
							...
							var responseD = JSON.parse(answer +  ']}');
						}
...
callback(0,'couldn\t do the job 240 accessories');
}
			});
		});
	},
	change_state: function(aid,iid,value,globalVariable, callback){
