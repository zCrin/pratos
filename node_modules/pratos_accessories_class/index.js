...
module.exports = {		
					...
					fs.readFile(path + "/conf/auto_updated_accessories.json", 'utf8', function(err, data){
						...
						for(var i = 0; i < out.length; i++){
							...
							answer += `{
									...
									"state" : "false"`;
							if(updated_accessories[aid].icon_on && updated_accessories[aid].icon_off){
	...
	change_state: function(globalVariable, callback){
		globalVariable.event.emit("accessories", "state:changing",{aid, iid,value});
		var aid = globalVariable.request.query.aid,
			iid = globalVariable.request.query.iid,
			value = globalVariable.request.query.value;
			fs.readFile(path + "/conf/settings.json", 'utf8', function(err, data){
	...
	update_accessory : function(globalVariable, callback){
		if(globalVariable.request.body.aid){
			var aid = globalVariable.request.body.aid;
			...
			fs.readFile(path + "/conf/auto_updated_accessories.json", 'utf8', function(err, data){
				...
				if(globalVariable.request.body.name){
					updated_accessories.accessories[aid].name = globalVariable.request.body.name;
				}
				if(globalVariable.request.body.manufacturer){
					updated_accessories.accessories[aid].manufacturer = globalVariable.request.body.manufacturer;
				}
				if(globalVariable.request.body.model){
					updated_accessories.accessories[aid].model = globalVariable.request.body.model;
				}
				if(globalVariable.request.body.icon_on){
					updated_accessories.accessories[aid].icon_on = globalVariable.request.body.icon_on;
				}
				if(globalVariable.request.body.icon_off){
					updated_accessories.accessories[aid].icon_off = globalVariable.request.body.icon_off;
				}
