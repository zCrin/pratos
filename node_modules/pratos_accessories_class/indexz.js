	...
	has_change_happened: function(globalVariable){
		cmd.get(
			...
			function(out){
				...
				fs.watch('/var/www/pratos/log/homebridge.log', 
					function(data) {
						if(data =='change'){
								...
								function (err, res) {
									if(res){
										...
										if(resArray[2] && !/Initializing|Getting|Requesting|HTTP get power function failed|HTTP set power function failed/i.test(resArray[2]) && resArray[2].trim() != ""){
											...
											module.exports.list_accessories(globalVariable, 'accessoriesChangeList');
											globalVariable.event.on("accessoriesChangeList", show_list_accessories);
										}
									}
									else{
											console.log(err);
		...
		function show_list_accessories (answer, data){
			...
			if(answer == "list:obtained"){
				globalVariable.event.removeListener("accessoriesChangeList", show_list_accessories);
				if(data != lastState){
					console.log('changed');
					console.log(data);
					lastState= data;
					globalVariable.event.emit("accessories", "change:detected",data);
					setTimeout(checkChangeAgain,10000);
				}
				else{
		...
		function checkChangeAgain(){
			if(Date.now() - lastEventChanged > 6000){
				lastEventChanged = Date.now();
				module.exports.list_accessories(globalVariable, 'accessoriesChangeList'+lastEventChanged);
				globalVariable.event.on("accessoriesChangeList"+lastEventChanged, show_list);
				setTimeout(function(){
					globalVariable.event.removeListener("accessoriesChangeList"+lastEventChanged, show_list);
				},4000);
			}
		}
		function show_list (answer, data){
			if(answer == "list:obtained"){
				console.log('mine');
				console.log("accessoriesChangeList"+lastEventChanged);
				globalVariable.event.removeListener("accessoriesChangeList"+lastEventChanged, show_list);
				if(data != lastState){
					lastState= data;
					globalVariable.event.emit("accessories", "change:detected",data);
...
update_rooms:function(user_id, globalVariable,callback){
	if(globalVariable[user_id].request.query.name && globalVariable[user_id].request.query.id){
		mongoDB.collection("rooms_list").update({_id: new ObjectID(globalVariable[user_id].request.query.id)},{ name:globalVariable[user_id].request.query.name},{upsert:true},function(){ });
		return callback('updated');
	}
	else{
		return callback('error');
...
delete_rooms:function(user_id, globalVariable,callback){
	...
	list_accessories: function(globalVariable, eventName){
		if(!eventName){
...
console.log(eventName +"has evrything");
...
answer += `, "room" : "` + updated_accessories[aid].room + '"';
						...
						console.log('getting ino');
						for(var z = 0; z < finalResponse.accessories.length; z++){
							console.log(object_state(settings.homebridge, z, globalVariable, eventName));
						}
						return true;
						}
						else{
							globalVariable.event.emit(eventName, "list:obtained","error");
							return false;
						}
...
function object_state(homebridge, number, globalVariable, eventName){

request({uri: homebridge.host +":"+ homebridge.port +"/characteristics?id=" + finalResponse.accessories[number].aid + '.' + finalResponse.accessories[number].iid ,headers:{'Content-Type':'application/json',authorization: homebridge.pin}},(err,res,state)=>{
...
if(finalResponse.accessories[number].state.length == 0){
finalResponse.accessories[number].state = 0;
}
var objectToTreat = finalResponse.accessories[number];
...
finalResponse.accessories[number].icon_on = icon_on;
finalResponse.accessories[number].icon_off = icon_off;
}
if(objectToTreat.category == "Temperature Sensor"){
						finalResponse.accessories[number].state = Math.round(objectToTreat.state) + " °C";
					}

		if(number == (finalResponse.accessories.length - 1)){
			has_been_fetched(homebridge, globalVariable, eventName);
		}
...
function has_been_fetched(homebridge, globalVariable, eventName ){
	var follow = true;
	var length = finalResponse.accessories.length;
	for(var b = 0; b < length; b++){
		if(finalResponse.accessories[b].state == "false"){
			object_state2(homebridge, b, globalVariable, eventName);
			return false;
		}
	}
	if(follow == true){
		
		globalVariable.event.emit(eventName, "list:obtained",JSON.stringify(finalResponse));
	}
	return true;
}
function object_state2(homebridge, number, globalVariable, eventName){
	setTimeout(function(){
