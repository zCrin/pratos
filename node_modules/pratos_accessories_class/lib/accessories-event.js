...
function detect_change(globalVariable){
	...
	function emitChange(data,globalVariable){
		if(data != lastState){
			...
			globalVariable.event.emit("accessories", "change:detected",data);
			 findSpecificChange(data,globalVariable);
			if(intervalObject != 0){
	...
	function findSpecificChange(data,globalVariable){
		var all_infos = JSON.parse(data).accessories;
		var Lastall_infos = JSON.parse(lastState).accessories;
		if(all_infos){
		...
		for(var z=0; z<f; z++){
			...
			if(!Lastall_infos[z] || val.state != Lastall_infos[z].state){
				globalVariable.event.emit(val.name+states[val.state],val);
			}
		}
		}else{
