...
globalFunctions ={},
globalVariableIn;
module.exports= analyse_request;
var colors = require('colors');
function analyse_request(text,globalVariable){
globalVariableIn= globalVariable;
	var regexDe = /^d( )' |de |la |l( )'/;
	text = text. trim().replace(regexDe,''). sansAccent();
	
			console.log("\Google Home : Connected to Pratos' databases.".magenta);
			mongoDB = globalVariable.database.mongoDB;
			globalVariable.database.mongoDB.collection("googleHome_action").findOne({'trigger': text}, function(error, data) {
				if(data){
					...
					if(actions[text.substr(0, text.indexOf(" ")).trim()]){
						...
						console.log('Google Home : "'+text+'" command not understood');
					}
				}
			});
...
globalFunctions.homebridge = function(text){
	Accessories.list_accessories(globalVariableIn,function(data){
		var firstWord = text.substr(0, text.indexOf(" ")).trim();
		...
		if(firstWord == 'allumer'){
			...
			mongoDB.collection("googleHome_action").update({trigger:text},{trigger:text, functionToCall:"homebridge",homebridgeAction:"turnOn",homebridgeObjectAid: data.accessories[n].aid, homebridgeObjectIid:data.accessories[n].iid},{upsert:true},function(err){});
		}
			...
			var n = homebridgeLookWhat(secondWorld, data);
				...
				mongoDB.collection("googleHome_action").update({trigger:text},{trigger:text, functionToCall:"homebridge",homebridgeAction:"turnOff",homebridgeObjectAid: data.accessories[n].aid, homebridgeObjectIid:data.accessories[n].iid},{upsert:true},function(err){});
		}
	});
};
...
function homebridgeChangeStateObject(aid,iid,state){
	Accessories .change_state(aid,iid,state, globalVariableIn,function(k){});
}
