...
module.exports = {
	...
	more_banning:function(user_id,globalVariable, ip){
		var user_agent = globalVariable[user_id].request.headers["user-agent"];
		fs.readFile(path + "/conf/auto_ip_ban.json", 'utf8', function(err, data){
			var ban = JSON.parse(data);
...
function addToFile(ban_file,callback){
	...
	fs.writeFile(path + "/conf/auto_ip_ban.json", JSON.stringify(ban_file),'utf8',(err) =>{
  if (err) throw err;
