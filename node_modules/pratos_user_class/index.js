...
module.exports = {
	...
	verify_connection: function(globalVariable, callback){
		globalVariable.event.emit("user", "verify_connection");
		fs.readFile(path + "/conf/auto_allowed_devices.json", 'utf8', function(err, data){
			if(globalVariable.request.session.user_allowed != true){

				var device_allowed = globalVariable.cookies.get("device_allowed");
				if(device_allowed){

					var allowed_devices = JSON.parse(data);
					if(array_search(device_allowed, allowed_devices.authorised_devices)){
						globalVariable.request.session.user_allowed = true;
					}
					else{
						globalVariable.request.session.user_allowed = false;
					}
				}
				else{
					globalVariable.request.session.user_allowed = false;
	...
	connect_user: function(globalVariable, callback){
		globalVariable.event.emit("user", "try_connection");
		var Ban = require("pratos_ban_class");
			var ip = getClientIP(globalVariable.request);
			Ban.is_banned(globalVariable, ip, function(res){
				if(res){
					...
					fs.readFile(path + "/conf/settings.json", 'utf8', function(err, data){
						...
						if(settings.website.code == globalVariable.request.body.password){
							...
							globalVariable.event.emit("user", "try_connection:not_connected");
							Ban.more_banning(globalVariable, ip);
							return callback(false);
						}
					});
				}
	...
	add_allowed_device: function(globalVariable, callback){
...
globalVariable.event.emit("user", "add_device");
			globalVariable.cookies.set('device_allowed', uniqId, {expires: new Date(253402300000000) });
			if(callback){
				return callback(true);
			}
		});
