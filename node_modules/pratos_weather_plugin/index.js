...
app.get('/homepage_weather.js', function(req, res) {
			res.setHeader('Content-Type', 'application/javascript');
			fs.readFile( __dirname + '/homepage_weather.js', 'utf8', function(err, data){
			res.end(data);
delete globalVariable[req.user_id];
});
...
console.log("http://api.openweathermap.org/data/2.5/weather?q="+location+"&APPID="+ data.apiKey);
request({uri: "http://api.openweathermap.org/data/2.5/weather?"+location+"&APPID="+ data.apiKey,timeout:10000},(err,ans,state)=>{
if(isJson(state)){
			res.end(state);
delete globalVariable[req.user_id];
}else{
...
{$set: { location: location}}, false, true);
