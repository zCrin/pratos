...
module.exports = {
    ...
    content: function (link) {
        ...
        link["weather_plugin"] = {
            "page": "/settings",
            "script": " <script src='/weather.js'></script>"
        };
        link["weather_plugin1"] = {
            "page": "/home",
            "script": " <script src='/homepage_weather.js'></script>"
        };
    ...
    get: function (app, globalVariable) {
        ...
        app.get('/weatherNeedsLocation', function (req, res) {
            user.is_connected(req.user_id, function (response, user_res) {
                if (response == true) {
                    mongoDB.collection("weatherPlugin").findOne({
                        job: "settings"
                    }, function (error, data) {
                        if (data) {
                            apiKey = data;
                    ...
                    mongoDB.collection("weatherPlugin").findOne({
                        ...
                        if (data) {
                            ...
                            if (!data.apiKey) {
                                ...
                                request({
                                    uri: "http://api.openweathermap.org/data/2.5/weather?" + location + "&lang=" + lang + "&units=" + unity + "&APPID=" + data.apiKey,
                                    timeout: 10000
                                }, (err, ans, state) => {
        ...
        app.get('/weatherApiSetInfos', function (req, res) {
            user.is_connected(req.user_id, function (response, user_res) {
                if (response == true) {
                    ...
                    mongoDB.collection("weatherPlugin").update({
                        job: "settings"
                    }, {
                        $set: {
                            location: location,
        ...
        app.get('/weatherApiDisconnect', function (req, res) {
            user.is_connected(req.user_id, function (response, user_res) {
                if (response == true) {
                    ...
                    mongoDB.collection("weatherPlugin").remove({
                        job: "settings"
                    });
            ...
            user.is_connected(req.user_id, function (response, user_res) {
                if (response == true) {
                    if (globalVariable[req.user_id].request.query.apikey) {
                        ...
                        request({
                            uri: "http://api.openweathermap.org/data/2.5/weather?q=London&APPID=" + momentKey,
                            timeout: 10000
                        }, (err, ans, state) => {
                            if (isJson(state)) {
                                state = JSON.parse(state);
    ...
    has_settings: function (globalvariable) {
        var y = {
            title: "Weather",
            buttonName: "weatherButton",
            buttonIcon: "<i class='fa fa-cloud'></i>",
            boxContent: "<h2>Weather</h2>"
        };
        return y;
    }
};
