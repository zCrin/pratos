...
function show_accessories(data) {
    ...
    $.each(all_infos, function (key, val) {
        ...
        var class_attr = 'material-shadow-3 accessory-buttons ' + form_display + ' ',
        img = false;
...
function update_accessory(aid, oldName) {
    var img = "";
    if ($('#' + aid + ' > img').length) {
        img = '<img aid="' + aid + '" src="' + $('#' + aid + ' > img').attr('src') + '"/>';
    }
    popup("<span class='material-popup-title '>Modifier l'accessoire</span><div id='popup-edit-header'><span id='change_pic'>" + img + "</span><input type='hidden' value='" + aid + "'id='acc_aid'/><div style='display: flex;  margin-left:5%;flex-direction: column;' id='div2'><div style='margin:0 0 0 4%;text-align: center;'><input type='text' spellcheck='false'id='update_accessory_name' value='" + $('#' + aid + ' > #accessory_name').text() + `'/></div><div class="input-div-box input-edit-accessory"><input type='text' placeholder="Modèle" id='update_accessory_model'spellcheck='false' /></div><div class="input-div-box input-edit-accessory"><input type='text' placeholder="Fabricant" id='update_accessory_manufacturer'spellcheck='false' /></div>
	<div class='material-select' ><span>Pièce</span><i class='fa fa-caret-down'></i></div></div></div>`, function () {
		//<label for='update_accessory_room'>Pièces : </label><select id='update_accessory_room'/><option>Aucune</option></select>
        inputbottombar();
		inputplaceholder();
    ...
    getRooms(function (rooms) {
        ...
        $('#update_accessory_room').on("selectmenuclose", function (event, ui) {
            $('.ui-button').blur();
        });
