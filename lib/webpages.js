...
function defaultPages(app, globalVariable, Accessories, style) {
    ...
    app.get('/css/:file.ejs', function (req, res) {
        user.is_connected(req.user_id, function (response, userData) {
            if (response == true) {
                style.getColors(userData, function (colors) {
    ...
    app.post('/connect_permanent', function (req, res) {
        user.connect_permanent(globalVariable[req.user_id].request.bodyPratos.id, req.headers['user-agent'], req.user_id, function (data) {
            delete globalVariable[req.user_id];
            if (data == "ok") {
                res.end('/home/?error=connect_autoOK');
                delete globalVariable[req.user_id];
            } else {
                res.end(data);
                delete globalVariable[req.user_id];
        ...
        user.is_connected(req.user_id, function (response, user_res) {
            if (response == true) {
                ...
                if (globalVariable[req.user_id].request.bodyPratos.file1 && globalVariable[req.user_id].request.bodyPratos.file2) {
                    var filePaths = [globalVariable[req.user_id].request.bodyPratos.file1.path, globalVariable[req.user_id].request.bodyPratos.file2.path];
    ...
    app.post('/change_style/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res) {
            if (response == true) {
                ...
                style.updateColor(globalVariable[req.user_id].request.bodyPratos, user_res, function () {
                    res.end('reload')
                })
    ...
    app.get('/get_userList/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'application/json');
                user.userList(client, function (list) {
                    res.end(list)
                });
	  ...
	  app.get('/get_groupsList/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'application/json');
                user.groupsList(client, function (list) {
                    res.end(list)
                });
    ...
    app.post('/admin_update_user/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                ...
                if (client.isAdmin) {
                    user.update_user(globalVariable[req.user_id].request.bodyPratos, function (respo) {
                        res.end(respo);
                        delete globalVariable[req.user_id];
                    });
                } else {
	...
	app.post('/admin_delete_user/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                ...
                if (client.isAdmin) {
                    user.delete_user(globalVariable[req.user_id].request.bodyPratos, function (respo) {
                        res.end(respo);
                        delete globalVariable[req.user_id];
                    });
                } else {
	...
	app.post('/admin_delete_group/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'text/html');

                if (client.isAdmin) {
                    user.delete_group(client,globalVariable[req.user_id].request.bodyPratos, function (respo) {
                        res.end(respo);
                        delete globalVariable[req.user_id];
                    });
                } else {
    ...
    app.post('/admin_unban_user/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'text/html');

                if (client.isAdmin) {
                    user.unban(globalVariable[req.user_id].request.bodyPratos.userID, function () {
                        res.end("true");
                        delete globalVariable[req.user_id];
                    });
                } else {
    ...
    app.post('/admin_ban_user/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'text/html');

                if (client.isAdmin) {
                    user.ban(globalVariable[req.user_id].request.bodyPratos, function () {
                        res.end("true");
                        delete globalVariable[req.user_id];
                    });
                } else {
    ...
    app.post('/admin_update_userPassword/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'text/html');

                if (client.isAdmin) {
                    user.update_user_password(globalVariable[req.user_id].request.bodyPratos, function (response) {
                        res.end(response);
                        delete globalVariable[req.user_id];
                    });
                } else {
    ...
    app.post('/admin_create_user/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'text/html');

                if (client.isAdmin) {
                    user.add_user(globalVariable[req.user_id].request.bodyPratos, function (response) {
                        res.end(response);
                        delete globalVariable[req.user_id];
                    });
...
app.post('/admin_create_group/', function (req, res) {
        user.is_connected(req.user_id, function (response, user_res, client) {
            if (response == true) {
                res.setHeader('Content-Type', 'text/html');

                if (client.isAdmin) {
                    user.add_group(globalVariable[req.user_id].request.bodyPratos, function (response) {
                        res.end(response);
                        delete globalVariable[req.user_id];
                    });
    ...
    app.get('/:adminURI/', function (req, res, next) {
        fs.stat(pathto + '/views/admin_' + req.params.adminURI + '.ejs', function (err, stat) {
            if (err == null) {
                user.is_connected(req.user_id, function (response, user_res, client) {
                    if (response == true) {
                        require(pathto + "/node_modules/pratos_navbar_class").construct(globalVariable.navbarPlugins, function (res_nav) {
                            res.setHeader('Content-Type', 'text/html');
                            if (req.params.adminURI == "home") {
