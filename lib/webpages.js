...
function defaultPages(app,globalVariable,Accessories,style){
	...
	app.get('/', function(req, res) {
		User.verify_connection(req.user_id,globalVariable, function(user_res){
			if(user_res != true){
				...
				res.render( pathto+'/views/index.ejs',{content: globalVariable.contentPlugins,page_name:"index"});
	...
	app.get('/css/:file.ejs', function(req, res) {
		...
		res.render(pathto+'/views/css/' + req.params.file + '.ejs', {variable: globalVariable, global_color:global_color, page_name: req.params.file,background_color: background_color});
		delete globalVariable[req.user_id];
	});
	app.post('/user_connect/', function(req, res) {
	
	if( globalVariable[req.user_id].request.bodyPratos.password){
			...
			if(user_res == true){
				...
				if(req.params.adminURI == "index"){
					require(pathto+"/node_modules/pratos_homepage_class").construct(globalVariable.homepagePlugins, function(res_homepage){
						res.render( pathto+'/views/admin_' + req.params.adminURI + '.ejs', {nav: res_nav, content: globalVariable.contentPlugins, homepage: res_homepage,page_name: "admin/"+req.params.adminURI});
					});
				}
				else if(req.params.adminURI == "settings"){
					require(pathto+"/node_modules/pratos_homepage_class").construct(globalVariable.homepagePlugins, function(res_homepage){
						res.render(pathto+'/views/admin_' + req.params.adminURI + '.ejs', {nav: res_nav, content: globalVariable.contentPlugins, page_name: "admin/"+req.params.adminURI , settings: globalVariable. pluginsSettings});
					});
				}
				else{
					res.render(pathto+'/views/admin_' + req.params.adminURI + '.ejs', {nav: res_nav, page_name: "admin/"+req.params.adminURI , content: globalVariable.contentPlugins});
	...
	app.get('/js/:file.ejs', function(req, res) {
		...
		res.render(pathto+'/views/js/' + req.params.file + '.ejs', {variable: globalVariable,  page_name: req.params.file});
		delete globalVariable[req.user_id];
	});
				...
				if(req.files.file1 && req.files.file2){
					...
					sharp(req.files.file1.data)
						...
						.then(function(outputBuffer) {
							fs.writeFile(pathto+'/static/img/icons/' + path1, outputBuffer);
					});
					sharp(req.files.file2.data)
						...
						.toBuffer()
						.then(function(outputBuffer) {
							fs.writeFile(pathto+'/static/img/icons/' + path2, outputBuffer);
					});
					fs.readFile(pathto+"/conf/auto_accessories_icon.json", 'utf8', function(err, data){
						var txt = JSON.parse(data);
						txt[path1] = '\/' + path1;
						txt[path2] = '\/' + path2;
						fs.writeFile(pathto+"/conf/auto_accessories_icon.json", JSON.stringify(txt));
						res.setHeader('Content-Type', 'application/json');
